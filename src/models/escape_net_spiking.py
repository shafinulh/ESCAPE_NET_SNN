...
class PoissonGenerator(nn.Module):
    ...
    def forward(self, input):
        ...
        out = torch.mul(
            torch.le(torch.rand_like(input), torch.abs(input) * 0.9).float(),
            torch.sign(input),
        )
        return out
    ...
    def neuron_init(self, x):
        ...
        for l in range(len(self.features)):
            ...
            if isinstance(self.features[l], nn.Conv2d):
                ...
                self.spiketrains[l] = torch.zeros(
                    self.timesteps,
                    self.batch_size
                    * self.features[l].out_channels
                    * self.width
        ...
        for l in range(len(self.classifier)):
            ...
            if isinstance(self.classifier[l], nn.Linear):
                self.mem[prev + l] = torch.zeros(
                    self.batch_size, self.classifier[l].out_features
                )
                self.spiketrains[prev + l] = torch.zeros(
                    self.timesteps, self.batch_size * self.classifier[l].out_features
                )

            elif isinstance(self.classifier[l], nn.Dropout):
                self.mask[prev + l] = self.classifier[l](
                    torch.ones(self.mem[prev + l - 2].shape)
                )
        ...
        for t in range(self.timesteps):
            ...
            for l in range(len(self.features)):
                ...
                if isinstance(self.features[l], (nn.Conv2d)):
                    ...
                    self.mem[l] = (
                        self.leak * self.mem[l] + self.features[l](out_prev) - rst
                    )
                    out_prev = out.clone()
            ...
            for l in range(len(self.classifier) - 1):
                ...
                if isinstance(self.classifier[l], (nn.Linear)):
                    ...
                    self.spike[prev + l] = self.spike[prev + l].masked_fill(
                        out.bool(), t - 1
                    )
                    self.mem[prev + l] = (
                        self.leak * self.mem[prev + l]
                        + self.classifier[l](out_prev)
                        - rst
                    )
                    out_prev = out.clone()
            ...
            if not find_max_mem:
                prev -= 1
                self.mem[prev + l + 1] = self.mem[prev + l + 1] + self.classifier[
                    l + 1
                ](out_prev)
        if find_max_mem:
            return max_mem
