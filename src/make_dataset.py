...
class Preprocessing_module:
    ...
    def Randomize_Train_and_getValidSet(
        self, training_data, training_labels, numspikes
    ):
        ...
        temp = random.sample(range(0, itemindex[0].shape[0]), numspikes)
        self.samples2 = temp
        validation_set = np.concatenate(
            (validation_set, training_data[itemindex[0][temp], :, :, :]), axis=0
        )
    ...
    def Randomize_Train_and_getValidSet2(
        self, training_data, training_labels, sample1, sample2, sample3, numspikes
    ):
        itemindex = np.where(training_labels == 1)
        temp = sample1
        validation_set = training_data[itemindex[0][temp], :, :, :]
